...
class PPO(PolicyGradient):
    ...
    def update_policy(self, observations, actions, advantages, old_logprobs):
        ...
        #     if r_theta[i] < 1-self.eps_clip :
        #         clip.append(1-self.eps_clip)
        #     elif r_theta[i] > 1+self.eps_clip :
        #         clip.append(1-self.eps_clip)
        #     else :
        #         clip.append(r_theta[i])
        # clip = torch.Tensor(clip)
        clip = torch.clamp(r_theta, min= 1-self.eps_clip, max= 1+self.eps_clip)
        loss = -torch.minimum(r_theta * advantages, clip * advantages).mean()
