	...
	while True:
		...
		for state in range(nS):
			...
			for probability, nextstate, reward, terminal in P[state][policy[state]]:
				V += reward + gamma * value_function[nextstate] * probability
			prev_value_function[state] = V
		value_chage = value_function - prev_value_function
		value_function = prev_value_function.copy()		
		if np.max(np.abs(value_chage))<tol:
			break
	############################
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
	...
	for state in range(nS):
		...
		for action in range(nA):
			V = 0
			for prob, next_s, reward, terminal in P[state][action]: # P[state][action] 
				V += reward + gamma * value_from_policy[next_s] * prob
			action_reward.append(V)
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=10e-3):
	...
	while True:
		...
		policy_change = np.sum(np.abs(policy - new_policy))
		policy = new_policy.copy()
		if policy_change == 0 :
			break
	print(f"policy done, iteration : {i} \n")
	############################
		...
		for state in range(nS):
			...
			for action in range(nA):
				...
				for prob, next_s, reward, terminal in P[state][action]:
					V += reward + gamma * temp_value_function[next_s] * prob
				action_reward.append(V)
			temp_value_function[state] = np.max(action_reward)
			policy[state] = np.argmax(action_reward)
		value_change = np.sum(np.abs(value_function - temp_value_function))
		value_function = temp_value_function.copy()
		if value_change < tol:
			break
	# policy = policy_improvement(P, nS, nA, value_function, policy)
	print(f"value done, iteration : {i} \n")
	############################
	return value_function, policy
